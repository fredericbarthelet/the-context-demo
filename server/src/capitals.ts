...
async function fetchAllCountriesMinimal(): Promise<RestCountryMinimal[]> {
  ...
  listCache = data.filter((c) => c.capital && c.capital.length > 0 && c.capitalInfo?.latlng);
  listCacheTime = now;
...
async function fetchCountryByCode(cca2: string): Promise<RestCountryFull> {
  ...
  const response = await fetch(`https://restcountries.com/v3.1/alpha/${cca2}?fields=${fields}`);
  if (!response.ok) {
    throw new Error(`RestCountries API error: ${response.status}`);
  }
...
async function fetchUnsplashPhotos(query: string, count: number = 4): Promise<Capital["photos"]> {
  try {
...
async function fetchWikipediaDescription(title: string): Promise<string | undefined> {
  try {
...
export async function getCapitalByCountryCode(cca2: string): Promise<Capital> {
  ...
  const [photos, capitalDesc, countryDesc] = await Promise.all([
    fetchUnsplashPhotos(`${capital} ${countryName} city landmark`),
    fetchWikipediaDescription(capital),
    fetchWikipediaDescription(countryName),
  ]);
  ...
  return {
    ...
    currencies: country.currencies
      ...
      : [],
    photos,
    wikipedia: { capitalDescription: capitalDesc, countryDescription: countryDesc },
  };
}
...
export async function getCapitalByName(capitalName: string): Promise<Capital> {
  ...
  const match = allCapitals.find((c) => c.name.toLowerCase() === capitalName.toLowerCase());
